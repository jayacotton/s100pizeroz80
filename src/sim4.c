...
#ifdef LOCALRAM
extern unsigned char localram[];
#define memwrt(a,b) localram[a]=b
#define memrdr(a) localram[a]
#else
#define memwrt(a,b) WriteRAM(a,b)
#define memrdr(a) ReadRAM(a)
#endif
...
{
	WritePort(C, A); /* possible bug here */
	return(12);
}
...
int op_undoc_out_null(void)	/* OUT (n),0 */
{
	...
	addr = memrdr(PC++);
	WritePort(addr, 0); /* NMOS, CMOS outputs 0xff */
	return(15);
}
...
int op_undoc_in_flags(void)	/* IN F,(n) */
{
	...
	addr = memrdr(PC++);
	tmp = ReadPort(addr); /* probably A on bus, but uncertain */
