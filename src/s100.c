...
void SetAddress(int location)
{
  int i;
  int k = 0;


   (location & 0b1       ) ? digitalWrite (DATA_A0, HIGH): digitalWrite (DATA_A0, LOW);
   (location & 0b10      ) ? digitalWrite (DATA_A1, HIGH): digitalWrite (DATA_A1, LOW);
   (location & 0b100     ) ? digitalWrite (DATA_A2, HIGH): digitalWrite (DATA_A2, LOW);
   (location & 0b1000    ) ? digitalWrite (DATA_A3, HIGH): digitalWrite (DATA_A3, LOW);
   (location & 0b10000   ) ? digitalWrite (DATA_A4, HIGH): digitalWrite (DATA_A4, LOW);
   (location & 0b100000  ) ? digitalWrite (DATA_A5, HIGH): digitalWrite (DATA_A5, LOW);
   (location & 0b1000000 ) ? digitalWrite (DATA_A6, HIGH): digitalWrite (DATA_A6, LOW);
   (location & 0b10000000) ? digitalWrite (DATA_A7, HIGH): digitalWrite (DATA_A7, LOW);

  digitalWrite (ADD_LOW, HIGH);	//Latch low address lines
  digitalWrite (ADD_LOW, LOW);


  (location & 0b100000000	) ? digitalWrite (DATA_A8, HIGH): digitalWrite (DATA_A8, LOW);
  (location & 0b1000000000      ) ? digitalWrite (DATA_A9, HIGH): digitalWrite (DATA_A9, LOW);
  (location & 0b10000000000     ) ? digitalWrite (DATA_A10, HIGH): digitalWrite (DATA_A10, LOW);
  (location & 0b100000000000    ) ? digitalWrite (DATA_A11, HIGH): digitalWrite (DATA_A11, LOW);
  (location & 0b1000000000000   ) ? digitalWrite (DATA_A12, HIGH): digitalWrite (DATA_A12, LOW);
  (location & 0b10000000000000  ) ? digitalWrite (DATA_A13, HIGH): digitalWrite (DATA_A13, LOW);
  (location & 0b100000000000000 ) ? digitalWrite (DATA_A14, HIGH): digitalWrite (DATA_A14, LOW);
  (location & 0b1000000000000000) ? digitalWrite (DATA_A15, HIGH): digitalWrite (DATA_A15, LOW);
	  digitalWrite (ADD_HIGH, HIGH);	//Latch high address lines
	  digitalWrite (ADD_HIGH, LOW);

  (location & 0b10000000000000000        ) ? digitalWrite (DATA_A16, HIGH): digitalWrite (DATA_A16, LOW);
  (location & 0b100000000000000000       ) ? digitalWrite (DATA_A17, HIGH): digitalWrite (DATA_A17, LOW);
  (location & 0b1000000000000000000	 ) ? digitalWrite (DATA_A18, HIGH): digitalWrite (DATA_A18, LOW);
  (location & 0b10000000000000000000     ) ? digitalWrite (DATA_A19, HIGH): digitalWrite (DATA_A19, LOW);
  (location & 0b100000000000000000000    ) ? digitalWrite (DATA_A20, HIGH): digitalWrite (DATA_A20, LOW);
  (location & 0b1000000000000000000000   ) ? digitalWrite (DATA_A21, HIGH): digitalWrite (DATA_A21, LOW);
  (location & 0b10000000000000000000000  ) ? digitalWrite (DATA_A22, HIGH): digitalWrite (DATA_A22, LOW);
  (location & 0b100000000000000000000000 ) ? digitalWrite (DATA_A23, HIGH): digitalWrite (DATA_A23, LOW);
	  digitalWrite (ADD_EXT, HIGH);	//Latch extended address lines
	  digitalWrite (ADD_EXT, LOW);
}

char
ReadRAM (int location)		// Set S100 bus address lines A0-A23 to a value (24 bits wide)
{				// Remember location is a int
  ...
  digitalWrite (CTL, LOW);	// GPIO0 Raspberry Pi Zero Pin 27       
	...
	k = 0;
  if(digitalRead(DATA_II0))  k = 1;
  if(digitalRead(DATA_II1))  k |= 0b10;
  if(digitalRead(DATA_II2))  k |= 0b100;   
  if(digitalRead(DATA_II3))  k |= 0b1000; 
  if(digitalRead(DATA_II4))  k |= 0b10000;
  if(digitalRead(DATA_II5))  k |= 0b100000;
  if(digitalRead(DATA_II6))  k |= 0b1000000;
  if(digitalRead(DATA_II7))  k |= 0b10000000;
...
void
WriteRAM (int location, int value)	// Set S100bus address lines A0-A23 to a value (24 bits wide)
...
char
ReadPort (int location)		// Set S100bus address lines A0-A15 to a value (24 bits wide)
{				// Remember location is a int
  int i;
  int k = 0;
...
void
WritePort (int location, int value)	// Set S100bus address lines A0-A15 to a value (24 bits wide)
{				// Remember location is a int
  int i;
  int k = 0;
...
int
CheckForInterrupts ()		// If there was an interrupt process it
{
  int k = 0;
...
void
ClearInterrupts ()		// Clear any pending interrupt
{
  EndBusCycle ();
...
void
EndBusCycle ()			// Clear all S100 Bus signals
{
  digitalWrite (READ_DATA, LOW);	// GPIO13 Raspberry Pi Zero Pin 33
...
void
S100_pSYNC ()			// Pulse pSYNC high & (pSTVAL Low)
{
